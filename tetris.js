...
var startY = 0;
var verticalArea = 20;
var horizantalArea = 10;
var coordinateArray = [...Array(verticalArea)].map(e => Array(horizantalArea).fill(0));
var curTetromino = [];
var tetrominoCopy;
var randomColor;
var tetrominos;
...
var tetrominos1 = [];
var randomTetromino;
var tetrominoColors = ['purple', 'cyan', 'blue', 'yellow', 'orange', 'lightgreen', 'red'];
...
document.querySelector('.score-board').innerHTML = 'SCORE:' + localStorage.getItem('score') + '<br/> HIGH: ' +
...
function getUserName() {
    ...
    var isSamewithOLdPlayerName = highScores.filter(function (item) { //sadece function return dönerek yapmaya çalış for'la(good partstan)
        return item.player == userName.player;
    })
...
function increaseScore() {
    ...
    if (localStorage.getItem('high score') === null || (!!localStorage.getItem('high score') &&
...
function moveDownAllStopped() {
    for (var i = 0; i < stoppedTetris.length; i++) {
        if (stoppedTetris[i][1] < limit) {
            ...
            document.querySelectorAll('.stopped')[i].style.left = coordinateArray[x][y].x + 'px';
            document.querySelectorAll('.stopped')[i].style.top = coordinateArray[x][y].y + 'px';
        }
    }
}
        ...
        if (count === horizantalArea) {
            ...
            for (var m = 0; m < deleteTetris.length; m++) {
                ...
                document.querySelectorAll('.stopped').forEach(function (value) {
                    if (value.style.left === coorX + 'px' && value.style.top === coorY + 'px') {
                        value.remove();
                    }
                });
            }
...
function changePosition() {
    for (var i = 0; i < curTetromino.length; i++) {
        ...
        document.querySelectorAll('.tetris')[i].style.left = coordinateArray[x][y].x + 'px';
        document.querySelectorAll('.tetris')[i].style.top = coordinateArray[x][y].y + 'px';
    }
}
